...
#         )
#     await state.clear()
@admin_ticket_router.callback_query(lambda c: c.data == "close_ticket")
async def complete_ticket(
    callback: CallbackQuery, state: FSMContext, l10n: FluentLocalization
):
    data = await state.get_data()
    ...
    # Безопасное получение языка пользователя
    if (
        task
        and task.user
        and hasattr(task.user, "language_code")
        and task.user.language_code
    ):
        user_language = task.user.language_code[:2]
    else:
        user_language = "ru"  # Язык по умолчанию

    # print(f"User language: {user_language}")  # Для отладки

    # Загружаем локализацию
    user_l10n = get_fluent_localization(user_language)
