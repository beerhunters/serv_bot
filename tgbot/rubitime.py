...
# import aiohttp
#
# from tgbot.config import RUBITIME_API_KEY
#
...
async def rubitime(method: str, extra_params: dict) -> str | None:
    ...
    if method == "get_record":
        ...
        logger.error("Неизвестный метод: %s", method)
        return None
            ...
            async with session.post(url, json=params, ssl=ssl_context) as response:
                if response.status == 200:
                    ...
                    if data.get("status") == "ok":
                        if method == "create_record":
                            record_id = data.get("data", {}).get("id")
                            logger.debug("Создано в Rubitime: ID %s", record_id)
                            return record_id
                        logger.debug("Запрос Rubitime успешен: %s", method)
                        return None
                    else:
                        logger.warning(
                            "Ошибка Rubitime: %s",
                    ...
                    logger.error(
                        "Ошибка HTTP: %s, ответ: %s",
                        response.status,
                        await response.text(),
                    )
                    return None
        except Exception as e:
            logger.error("Исключение при запросе к Rubitime: %s", str(e))
            return None
