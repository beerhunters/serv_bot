...
# from aiogram import BaseMiddleware
# from aiogram.types import Message, CallbackQuery
# from tools.fluent_loader import get_fluent_translation  # Новый импорт
#
#
# class L10nMiddleware(BaseMiddleware):
#     def __init__(self, default_locale: str = "ru"):
...
#
#         try:
#             # Передаем функцию перевода в data
#             data["t"] = get_fluent_translation(language_code[:2])
#         except FileNotFoundError:
#             # Если файл локализации не найден, используем дефолтный язык
#             data["t"] = get_fluent_translation(self.default_locale)
#
#         return await handler(event, data)
