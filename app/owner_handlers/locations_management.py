...
from app.database.requests import get_all_locations, create_location, delete_location, get_location_by_id
...
async def manage_locations(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.edit_text(text="💠 Выберите действие:", reply_markup=await kb.manage_locations())
...
async def list_promocodes(callback: CallbackQuery, state: FSMContext):
    ...
    if current_page_locations:
        ...
        for i, location in enumerate(current_page_locations, 1):
            text += (
                f"{i}. <code>{location.name}</code>\n\n"
            )
        keyboard = await kb.locations(
            "my_location_page_", "manage_locations", page, len(locations), page_size, end_index)
    else:
        text = (
            '📨 Пу-пу-пу:\n\n'
...
async def add_new_location(callback: CallbackQuery, state: FSMContext):
...
async def add_location(message: Message, state: FSMContext):
    ...
    await message.answer(f"📨 Создана локация: <code>{location}</code>", reply_markup=await kb.owner_main())
    await state.clear()


@owner_locations_management.callback_query(F.data == "delete_location")
async def delete_loc(callback: CallbackQuery, state: FSMContext):
    await state.set_state(LocationsManagement.del_location)
    list_of_locations = await get_all_locations()
    await callback.message.edit_text("Выберите локацию для удаления: ",
                                     reply_markup=await kb.list_locations(list_of_locations))
    await callback.answer()


@owner_locations_management.callback_query(F.data.startswith("location_"))
async def del_location(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"📨 Удалена локация: <code>{name}</code>", reply_markup=await kb.owner_main())
    await callback.answer()
    await state.clear()
