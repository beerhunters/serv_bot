...
# async def tickets(
#     cd_next_prev, cd_back, page: int, tickets_list: int, page_size: int, end_index: int, l10n: FluentLocalization
# ) -> InlineKeyboardMarkup:
#     buttons_data = []
#     if tickets_list > page_size:
#         if page > 1:
...
#             (
#                 f"space_{tariff.id}"
#                 if tariff.purpose in {l10n.format_value("meeting_room"), l10n.format_value("amphitheater")}
#                 else f"tariff_{tariff.id}"
#             ),
#             "callback",
#         )
#         for tariff in all_tariffs
#         if not (
#             tariff.name == l10n.format_value("test_day") and user and user.successful_bookings > 0
#         )
#     ]
#     return await create_buttons(buttons_data, back_callback_data="main_menu", l10n=l10n)
...
# async def payment(confirmation_url: str, amount: int, l10n: FluentLocalization) -> InlineKeyboardMarkup:
#     buttons_data = [
#         (f"Оплатить {amount} рублей", confirmation_url, "url"),
#         ("Отмена", "cancel_pay", "callback"),
#     ]
...
# async def time_intervals(current_date: datetime, l10n: FluentLocalization) -> InlineKeyboardMarkup:
#     now = datetime.now()
#     start_hour = 9
#     end_hour = 21
#
...
# async def duration_options(current_value: int = 1, l10n: FluentLocalization) -> InlineKeyboardMarkup:
#     buttons_data = [
#         ("-", "duration:decrease", "callback"),
#         (f"{current_value} час(а)", f"duration:{current_value}", "callback"),
#         ("+", "duration:increase", "callback"),
#         (l10n.format_value("confirm_btn"), f"confirm_duration:{current_value}", "callback"),
#     ]
...
# async def printers_list(printers: dict[str, str], l10n: FluentLocalization) -> InlineKeyboardMarkup:
#     buttons_data = [
#         (printer_name.replace("_", " "), f"select_printer:{printer_name}", "callback")
#         for printer_name in printers.keys()
#     ]
...
# async def question(question_id, answer_options, l10n: FluentLocalization) -> InlineKeyboardMarkup:
#     indexed_options = list(enumerate(answer_options, start=1))
#     shuffle(indexed_options)
#     buttons_data = [
#         (option, f"answer_{question_id}_{index}", "callback")
