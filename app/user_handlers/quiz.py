...
from app.database.requests import get_all_quizzes, get_questions_for_quiz, record_quiz_result, get_adjustments
...
async def start_quiz(callback: CallbackQuery, state: FSMContext):
    ...
    if not quizzes:
        try:
            await callback.message.answer("В данный момент нет доступных квизов.", reply_markup=await kb.user_main())
    ...
    try:
        await callback.message.edit_text("Выберите квиз из списка:", reply_markup=await kb.quiz_list(quizzes))
    except TelegramBadRequest as e:
        if str(e) != "Bad Request: message to delete not found":
            raise e
...
async def quiz_selected(callback: CallbackQuery, state: FSMContext):
    quiz_id = callback.data.split('_')[1]  # Ожидаем, что data будет вида 'quiz_<quiz_id>'
    quizzes = await get_all_quizzes()
    ...
    if not quiz:
        await callback.message.answer(
            ...
            reply_markup=await kb.quiz_list(quizzes)
        )
        return
...
async def ask_question(callback: CallbackQuery, state: FSMContext):
    ...
    if question['photo_url']:
        ...
        message = await callback.message.answer_photo(
            ...
            reply_markup=await kb.question(question['id'], question['answer_options'])
        )
...
@quiz_router.callback_query(QuizStates.answering_questions, F.data.startswith('answer_'))
async def handle_answer(callback: CallbackQuery, state: FSMContext):
    ...
    selected_answer_text = question['answer_options'][selected_answer_index].strip().lower()
...
async def finish_quiz(callback: CallbackQuery, state: FSMContext):
    ...
    await record_quiz_result(user_id=user_id, quiz_id=quiz_id, score=score, completed_at=completed_at)
